...
public sealed class PerlLanguagePack : ILanguagePack, IOutlineProvider
{
    private static readonly string[] Extensions = [".pl", ".pm", ".t", ".pod"];
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        PerlLexer lexer = new(text, diagnostics, cancellationToken);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial)
            isPartial = true;

        LineMap map = new(text);
        PerlParser parser = new(text, tokens, map, diagnostics, cancellationToken);
