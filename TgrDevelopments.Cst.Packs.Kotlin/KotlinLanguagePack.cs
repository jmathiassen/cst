...
public sealed class KotlinLanguagePack : ILanguagePack, IOutlineProvider
{
    private static readonly string[] Extensions = [".kt", ".kts"];
    private static readonly string[] NamePatterns = Array.Empty<string>();

    public string LanguageId => "kotlin";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        KotlinLexer lexer = new(text, diagnostics, cancellationToken);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial)
            isPartial = true;
        LineMap map = new(text);
        KotlinParser parser = new(text, tokens, map, diagnostics, cancellationToken);
