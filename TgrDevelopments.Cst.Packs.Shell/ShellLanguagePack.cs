...
/// <summary>Shell language pack (lexer → recursive-descent outline).</summary>
public sealed class ShellLanguagePack : ILanguagePack, IOutlineProvider
{
    private static readonly string[] Extensions = [".sh", ".bash", ".zsh", ".ksh"];
    private static readonly string[] NamePatterns = Array.Empty<string>();

    /// <inheritdoc />
    public string LanguageId => "shell";
    ...
    {
        ShLexer lexer = new(text, diagnostics, ct);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial)
            isPartial = true;
        ShParser parser = new(text, tokens, diagnostics, ct);
