...
/// <summary>TypeScript language pack (lexer → recursive-descent outline).</summary>
public sealed class TypeScriptLanguagePack : ILanguagePack, IOutlineProvider
{
    private static readonly string[] Extensions = [".ts", ".tsx", ".mts", ".cts"];
    ...
    {
        TsLexer lexer = new(text, diagnostics, ct);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial)
            isPartial = true;

        TsParser parser = new(text, tokens, diagnostics, ct);
