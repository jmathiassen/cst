...
/// <summary>GraphQL language pack (lexer → recursive-descent outline).</summary>
public sealed class GraphqlLanguagePack : ILanguagePack, IOutlineProvider
{
    private static readonly string[] Extensions = [".graphql", ".gql"];
    private static readonly string[] NamePatterns = Array.Empty<string>();

    /// <inheritdoc />
    public string LanguageId => "graphql";
    /// <inheritdoc />
    ...
    {
        GqlLexer lexer = new(text, diagnostics, ct);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial) isPartial = true;
        GqlParser parser = new(text, tokens, diagnostics, ct);
