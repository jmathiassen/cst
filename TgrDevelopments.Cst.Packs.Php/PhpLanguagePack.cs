...
public sealed class PhpLanguagePack : ILanguagePack, IOutlineProvider
{
    private static readonly string[] Extensions = [".php", ".phtml"];
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        PhpLexer lexer = new(text, diagnostics, cancellationToken);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial)
            isPartial = true;
        LineMap map = new(text);
        PhpParser parser = new(text, tokens, map, diagnostics, cancellationToken);
