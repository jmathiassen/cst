...
public sealed class OdinLanguagePack : ILanguagePack, IOutlineProvider
{
    private static readonly string[] Extensions = [".odin"];
    private static readonly string[] NamePatterns = Array.Empty<string>();

    public string LanguageId => "odin";
    public string DisplayName => "Odin";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        OdinLexer lexer = new(text, diagnostics, cancellationToken);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial)
            isPartial = true;
        LineMap map = new(text);
        OdinParser parser = new(text, tokens, map, diagnostics, cancellationToken);
