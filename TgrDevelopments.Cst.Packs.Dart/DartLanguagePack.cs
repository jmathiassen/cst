...
public sealed class DartLanguagePack : ILanguagePack, IOutlineProvider
{
    private static readonly string[] Extensions = [".dart"];
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        DartLexer lexer = new(text, diagnostics, cancellationToken);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial)
            isPartial = true;
        LineMap map = new(text);
        DartParser parser = new(text, tokens, map, diagnostics, cancellationToken);
