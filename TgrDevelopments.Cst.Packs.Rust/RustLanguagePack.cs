...
public sealed class RustLanguagePack : ILanguagePack, IOutlineProvider
{
    private static readonly string[] Extensions = [".rs"];
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        RustLexer lexer = new(text, diagnostics, cancellationToken);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial)
            isPartial = true;
        LineMap map = new(text);
        RustParser parser = new(text, tokens, map, diagnostics, cancellationToken);
