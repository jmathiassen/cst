...
public sealed class SwiftLanguagePack : ILanguagePack, IOutlineProvider
{
    private static readonly string[] Extensions = [".swift"];
    private static readonly string[] NamePatterns = Array.Empty<string>();

    public string LanguageId => "swift";
    public string DisplayName => "Swift";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        SwiftLexer lexer = new(text, diagnostics, cancellationToken);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial)
            isPartial = true;
        LineMap map = new(text);
        SwiftParser parser = new(text, tokens, map, diagnostics, cancellationToken);
