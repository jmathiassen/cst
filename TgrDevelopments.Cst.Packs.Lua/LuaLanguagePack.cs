...
public sealed class LuaLanguagePack : ILanguagePack, IOutlineProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        LuaLexer lexer = new(text, diagnostics, cancellationToken);
        IReadOnlyList<Token> tokens = lexer.Tokenize();
        if (lexer.IsPartial)
            isPartial = true;
        LineMap map = new(text);
        LuaParser parser = new(text, tokens, map, diagnostics, cancellationToken);
